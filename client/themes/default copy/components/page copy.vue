...
v-app(
  ...
  v-navigation-drawer(
    ...
    vue-scroll(:ops="scrollStyle")
      nav-sidebar(
        :color="$vuetify.theme.dark ? `grey darken-4-d4` : `secondary`",
  ...
  v-main(ref="content")
    ...
    v-container.pa-0(fluid)
      v-row.page-header-section(no-gutters, align-content="center")
        v-col.page-col-content.is-page-header(
          ...
          :class="$vuetify.rtl ? `pr-8` : `pl-8`"
        )
          .page-header-headings
            .headline(style="font-weight: 600") {{ title }}
            .caption(style="margin-top: 0.5rem") {{ description }}
          .page-edit-shortcuts(
    ...
    v-divider
    v-container.pl-8.pt-4(fluid, grid-list-xl)
      v-layout(row)
        v-flex.page-col-sd(
              ...
              template(v-for="(tocItem, tocIdx) in tocDecoded")
                ...
                template(v-for="tocSubItem in tocItem.children")
                  v-list-item(
                    ...
                    v-list-item-title.px-3.caption.grey--text(
          ...
          v-card.page-comments-card.mb-5(
            ...
            .pa-5
              .overline.pb-2.blue-grey--text.d-flex.align-center(
                ...
                v-tooltip(right, v-if="commentsPerms.write")
                  template(v-slot:activator="{ on }")
                    v-btn.ml-2(
                      ...
                      v-icon(
                        :color="$vuetify.theme.dark ? `blue-grey lighten-1` : `blue-grey darken-2`",
                        dense
                      ) mdi-comment-plus
        ...
        v-flex.page-col-content(
          ...
          v-tooltip(
            :right="$vuetify.rtl",
            :left="!$vuetify.rtl",
            v-if="hasAnyPagePermissions && editShortcutsObj.editFab"
          )
...
export default {
  ...
  computed: {
            ...
            result.push({
              path:
                _.get(
                  _.last(result),
                  'path',
                  this.locales.length > 0 ? `/${this.locale}` : ''
                ) + `/${value}`,
              name: value
                .split('-')
    ...
    editMenuExternalUrl() {
      if (
        this.editShortcutsObj.editMenuBar &&
        this.editShortcutsObj.editMenuExternalBtn
      ) {
        return this.editShortcutsObj.editMenuExternalUrl.replace(
          '{filename}',
          this.filename
        )
      } else {
  ...
  created() {
    ...
    if (this.editShortcuts) {
      this.$store.set(
        'page/editShortcuts',
        JSON.parse(Buffer.from(this.editShortcuts, 'base64').toString())
      )
    }
      ...
      this.$refs.container
        ...
        .forEach((el) => {
          el.onclick = (ev) => {
            ...
            ev.stopPropagation()
            this.$vuetify.goTo(
              decodeURIComponent(ev.currentTarget.hash),
              this.scrollOpts
            )
          }
        })
